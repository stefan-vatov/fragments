...
The app supports search across titles, tags, collections, and note bodies; Starred and Recent views; collection and tag filtering; Markdown editing and preview; copy; rename; move; and trash. The editor opens beside a temporary scratchpad for drafting or pasting text. Toggle **Scratchpad** beside Edit and Preview to give the fragment the full width. Its text is shared across fragments, stays only in memory, and disappears when the Fragments window closes. Use Ctrl+K (⌘K on macOS) for the command palette and Ctrl+S (⌘S on macOS) to save a fragment. Fragment edits also save after a short pause. Fragments reads changes made to the Markdown files outside the app and avoids overwriting a note changed while a draft was open.
