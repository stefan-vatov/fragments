...
The app supports search across titles, tags, collections, and note bodies; Starred and Recent views; collection and tag filtering; syntax-highlighted editing and preview; copy; rename; move; and trash. The search field in the fragment header finds titles and tags across the entire library, even while a collection or tag is selected. Use the list search to search within the current view, including note bodies. Both fields use the same fuzzy title and tag matching. New fragments use **Plain text**. Choose a language with the searchable Language field to highlight code in both Edit and Preview. Choose **Markdown** to highlight its source in Edit and render formatted Markdown in Preview. The editor opens beside a scratchpad for drafting or pasting text. Toggle **Scratchpad** beside Edit and Preview to give the fragment the full width. Its text is shared across fragments and saved as `_fragments-scratchpad.md` in the selected Fragments folder, so it survives closing the window. The scratchpad file stays out of the fragment list. Use Ctrl+K (⌘K on macOS) for the command palette and Ctrl+S (⌘S on macOS) to save. Edits also save after a short pause. Fragments reads changes made to the Markdown files outside the app and avoids overwriting a note changed while a draft was open.
